...
from .reason_codes import (
    ALLOC_BLOCKED_BY_PORTFOLIO_FIT,
    ALLOC_HIGH_PRIORITY,
    ALLOC_LOW_PRIORITY,
    ALLOC_MACRO_DUPLICATION,
    ALLOC_PORTFOLIO_HEAT_CAP,
    ALLOC_RESIZED_FOR_CLUSTER,
    ALLOC_RESIZED_FOR_CONCENTRATION,
    ALLOC_RISK_BUDGET_EXHAUSTED,
    ALLOC_USD_CONCENTRATION_CAP,
)


class PortfolioAllocator:
    def __init__(self, config: ControlPlaneConfig | None = None, correlation_engine: CorrelationEngine | None = None) -> None:
    ...
    def score_candidate_priority(self, candidate: AllocationCandidate, regime_decision: RegimeDecision, event_decision: EventDecision, execution_decision: ExecutionDecision, edge_snapshot: dict | None) -> float:
        base = (
            candidate.expected_value * self.config.ALLOC_PRIORITY_EV_WEIGHT
            + candidate.execution_score * self.config.ALLOC_PRIORITY_EXEC_WEIGHT
            + candidate.edge_score * self.config.ALLOC_PRIORITY_EDGE_WEIGHT
            + candidate.regime_score * self.config.ALLOC_PRIORITY_REGIME_WEIGHT
            + candidate.event_score * self.config.ALLOC_PRIORITY_EVENT_WEIGHT
        )
    ...
    def allocate(self, candidate_pool: list[AllocationCandidate], portfolio_state: PortfolioStateSnapshot) -> AllocationDecision:
        ...
        for c in sorted(candidate_pool, key=lambda x: (-(x.priority_score or 0.0), x.candidate_id)):
            ...
            if instrument_count[c.instrument] >= self.config.ALLOC_MAX_INSTRUMENT_DUPLICATION:
                blocked.append(c.candidate_id); reasons.append(ALLOC_BLOCKED_BY_PORTFOLIO_FIT); continue

            risk = c.risk_fraction_requested
            cluster = c.macro_cluster_key or "other"
            if cluster_alloc[cluster] + risk > self.config.ALLOC_MAX_SINGLE_MACRO_CLUSTER_RISK:
                if self.config.ALLOC_ENABLE_RESIZING:
                    risk = max(0.0, self.config.ALLOC_MAX_SINGLE_MACRO_CLUSTER_RISK - cluster_alloc[cluster])
                    if risk <= 0:
            ...
            usd_change = c.currency_exposure_map.get("USD", 0.0)
